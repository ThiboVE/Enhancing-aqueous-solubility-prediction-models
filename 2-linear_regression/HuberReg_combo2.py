...
def main() -> None:
    ...
    BASE = Path.parent(__file__)
    file_name = Path(__file__).stem + f"_{fold_id}"

    df_file = BASE / "processed_dataset_wo_metals_w_even_more_qm2.csv"
    splits_file = BASE / "splits_combo2.pkl"
    log_file = BASE / file_name + ".log"
    result_file = BASE / file_name + "_results.pkl"
    ...
    scores: dict[str, Any] = process_fold(X_train, X_test, y_train, y_test, grid, logger=logger)
