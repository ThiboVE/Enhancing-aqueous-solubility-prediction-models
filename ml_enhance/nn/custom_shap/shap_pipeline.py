...
from ml_enhance.nn.chemprop_pipeline import (
    Config,
    atom_features,
    bond_features,
    mol_features,
)
from ml_enhance.nn.featurizer import (
...
def get_predictions(
    ...
    atom_featurizer = (
        CustomMultiHotAtomFeaturizer.from_custom(
            keep_features=unpacked["keep_atom_features"],
        )
        if cfg.use_custom_atom_featurizer
        else CustomMultiHotAtomFeaturizer.from_base(
            keep_features=unpacked["keep_atom_features"],
        )
    )
...
def determine_shap(
    ...
    p_get_predictions = partial(get_predictions, cfg=cfg)

    model_wrapper = SHAPModelWrapper(
        datapoint=datapoint, mask_config=mask_config, mpnn=mpnn, trainer=trainer, get_predictions=p_get_predictions
    )
...
# def run_fold(
#     fold_id: int, outer_train_ids: list, outer_test_ids: list, mpnn: models.MPNN, p_build_datasets: Callable, cfg: Config
# ) -> dict[str, Any]:
#     _, outer_test_dset, _ = p_build_datasets(outer_train_ids, outer_test_ids)
